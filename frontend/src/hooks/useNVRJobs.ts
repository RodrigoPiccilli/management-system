...
/**
 * @function useNVRJobs
 * Custom React hook for managing NVR job data.
 * 
 * Fetches NVR jobs from the API, caches them in localStorage, and provides
 * loading state for UI feedback. Supports force-refreshing the data.
 * 
 * @returns {UseJobsReturn<NVRJob>} An object containing:
 *  - jobs: Array of `NVRJob` items currently loaded.
 *  - loading: Boolean indicating whether data is being fetched.
 *  - fetchJobs: Function to refresh the NVR job data, bypassing the cache.
 */
export function useNVRJobs(): UseJobsReturn<NVRJob> {
    ...
    /**
     * @function fetchJobs
     * Fetches NVR jobs from API or cache and updates state accordingly.
     * 
     ...
     * Convenience wrapper to force-refresh NVR job data.
     */
